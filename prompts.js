(() => {
  ...
  const TEXT_SYSTEM = [
    ...
    "返回一个 JSON 对象，不要 Markdown。字段：headline（概括核心事件，应明确主体与核心争议）、event_summary（连续正文，写明平台用户“@用户名”、事件经过等）、opinion_points（3至4个字符串，每项必须以“部分网民”开头并归纳一类评论观点）。",
    "输出示例：",
    JSON.stringify({
